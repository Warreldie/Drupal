 ...
 * @endcode
 */
$settings['hash_salt'] = '6AnRMVR2pmPIIsebeQ8T3RDloN_PFlUvUYvB4EKpDg-Vw-hteJwPlWEsGRpAVkgg2QNWtyHNKA';
 ...
 *
 * By default managed files that aren't used anymore won't be deleted. You can
 * change this behaviour by uncommenting the next line. In that case the file
 * usage will be set to 0 and the file will be deleted the next cron run.
 */
...
if (!\Drupal\Core\Installer\InstallerKernel::installationAttempted() && extension_loaded('redis')) {
  ...
  $settings['cache_prefix'] = ''; // Optional prefix for cache entries
  ...
  // Set Redis to not get the cache_form.
...
/**
 * Take reverse proxies into account. This is required to let Drupal know it is
 * being accessed through HTTPS. Remove this when there are no reverse proxies.
 * More information can be found higher up in the settings file.
 */
